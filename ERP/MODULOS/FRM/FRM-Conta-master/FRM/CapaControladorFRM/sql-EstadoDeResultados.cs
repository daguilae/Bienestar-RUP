...
namespace CapaControladorCONTA
{
	public class sql_EstadoDeResultados
	{
		...
		public OdbcDataAdapter Ingresos(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
						 "WHERE id_balance ="+idLibro+" AND '4.1' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
						 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";
			OdbcDataAdapter dataTable = new OdbcDataAdapter(sql, conectar.conexion("ERP"));
			return dataTable;
		}
		public OdbcDataAdapter Compras(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
			 "WHERE id_balance =" + idLibro + " AND '5.1' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
			 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";
			OdbcDataAdapter dataTable = new OdbcDataAdapter(sql, conectar.conexion("ERP"));
			return dataTable;
		}
		public OdbcDataAdapter Gastos(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
			 "WHERE id_balance =" + idLibro + " AND '6.1' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
			 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";
		...
		public OdbcDataAdapter Patrimonio(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
			 "WHERE id_balance =" + idLibro + " AND '3.1' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
			 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";

			OdbcDataAdapter dataTable = new OdbcDataAdapter(sql, conectar.conexion("ERP"));
			return dataTable;
		}
		public OdbcDataAdapter PasivoNC(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
			 "WHERE id_balance =" + idLibro + " AND '2.2' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
			 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";
			OdbcDataAdapter dataTable = new OdbcDataAdapter(sql, conectar.conexion("ERP"));
			return dataTable;
		}
		...
		{
			int i = 0;
			OdbcCommand command = new OdbcCommand("SELECT * FROM estado_de_resultados_detalles WHERE id_estado_de_resultado="+idMayor+";", conectar.conexion("ERP"));
			OdbcDataReader reader = command.ExecuteReader();
			while (reader.Read())
			{
				i++;
		...
		public string llenarComboDiarios()
		{
			string Combo = "";
			int numeroFIlas = 0;
			OdbcCommand command = new OdbcCommand("SELECT id_balance FROM balances WHERE estado =1;", conectar.conexion("ERP"));
			OdbcDataReader reader = command.ExecuteReader();
			if (reader.HasRows)
			{
				while (reader.Read())
		...
		{
			string id = "";
			OdbcCommand command = new OdbcCommand("SELECT MAX(id_estado_de_resultado) FROM estado_de_resultados;", conectar.conexion("ERP"));
			OdbcDataReader reader = command.ExecuteReader();
			if (reader.Read())
			{
