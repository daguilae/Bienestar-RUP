...
namespace CapaControladorCONTA
{
	public class sql_BalanceGeneral
	{
		...
		public OdbcDataAdapter ActivoC(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
						 "WHERE id_balance ="+idLibro+" AND '1.1' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
						 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";
			OdbcDataAdapter dataTable = new OdbcDataAdapter(sql, conectar.conexion("ERP"));
			return dataTable;
		}
		public OdbcDataAdapter ActivoNC(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
			 "WHERE id_balance =" + idLibro + " AND '1.2' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
			 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";
			OdbcDataAdapter dataTable = new OdbcDataAdapter(sql, conectar.conexion("ERP"));
			return dataTable;
		}
		public OdbcDataAdapter PasivoC(string idLibro)
		{
			string sql = "SELECT cuenta_contable, IF(deudor>0, deudor, acreedor) FROM balances_detalles  " +
			 "WHERE id_balance =" + idLibro + " AND '2.1' = (SELECT id_tipo_cuenta from cuentas WHERE nombre = cuenta_contable) " +
			 "ORDER BY (SELECT id_cuenta from cuentas WHERE nombre = cuenta_contable)";
		...
		{
			int i = 0;
			OdbcCommand command = new OdbcCommand("SELECT * FROM balance_general_detalles WHERE id_balance_general="+idMayor+";", conectar.conexion("ERP"));
			OdbcDataReader reader = command.ExecuteReader();
			while (reader.Read())
			{
				i++;
		...
		{
			string id = "";
			OdbcCommand command = new OdbcCommand("SELECT MAX(id_balance_general) FROM balance_general_encabezados;", conectar.conexion("ERP"));
			OdbcDataReader reader = command.ExecuteReader();
			if (reader.Read())
			{
